...
try:
    from src.condition_analyzer.condition_extractor import ConditionExtractor
    from src.condition_analyzer.condition_parser import ConditionParser
    from src.condition_analyzer.condition_solver import ConditionSolver
except ImportError:
...
def parse_args():
    ...
    parser.add_argument("--source-dir", "-s", type=str, required=True, help="분석할 소스 코드 디렉토리 경로")

    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="condition_analysis_result.json",
    ...
    parser.add_argument(
        "--max-iterations", "-m", type=int, default=100, help="조건식 솔버의 최대 반복 횟수 (기본값: 100)"
    )
...
def analyze_conditions(source_dir: str, file_extensions: List[str], max_iterations: int) -> Dict[str, Any]:
    ...
    for i, condition in enumerate(all_conditions):
        ...
        try:
            ...
            condition_results[condition] = {"error": str(e), "satisfying_values": {}, "non_satisfying_values": {}}
...
def main():
    ...
    extensions = args.extensions if args.extensions else [".c", ".cpp", ".h", ".hpp"]
    max_iterations = args.max_iterations if args.max_iterations else 100
    output_path = args.output if args.output else "condition_analysis_result.json"
    verbose = args.verbose if args.verbose else False

    setup_logging(verbose)

    logging.info(f"소스 디렉토리: {source_dir}")
    logging.info(f"파일 확장자: {extensions}")

    result = analyze_conditions(source_dir, extensions, max_iterations)
