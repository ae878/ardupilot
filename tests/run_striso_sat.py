...
condition = Condition("src/adapter/striso/condition_analysis_result.json")
config = ConfigFactory("src/adapter/striso/macros.json", condition=condition)
...
while True:
    for target_thread in target_thread_functions:
        ...
        fuzzer.mutate([target_thread], methods=["sat-validate"])
